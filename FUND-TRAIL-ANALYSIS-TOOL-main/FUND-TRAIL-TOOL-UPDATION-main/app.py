...
# Create a file handler in a safe location
if os.name == 'nt':
...
if not db_url:
    db_url = f"sqlite:///{os.path.join(app.root_path, 'fundtrail.db')}"
app.config['SQLALCHEMY_DATABASE_URI'] = db_url
# Configure a separate SQLite database for POH refund details persistence
# Use absolute path to ensure it is created in the project root
poh_db_path = os.path.join(app.root_path, 'poh_refund_details.db')
kyc_db_path = os.path.join(app.root_path, 'kyc_details.db')
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            for account_number in account_numbers:
                # Fetch transactions
                logger.info(f"Generating letter for ack_no={ack_no}, account={account_number}, is_poh={is_poh}")
                if is_poh:
                    transactions = Transaction.query.filter(
                        Transaction.ack_no == ack_no,
                        Transaction.put_on_hold_txn_id != None,
                        Transaction.put_on_hold_txn_id != '',
                        or_(Transaction.account_number == account_number, Transaction.to_account == account_number)
                    ).all()
                else:
                ...
                layer_label = "Unknown layer"
                date_minus_6_str = from_date
                
                if transactions and len(transactions) > 0:
                    for t in transactions:
                        if hasattr(t, 'layer') and t.layer:
                            suspect_layer = str(t.layer)
                            layer_label = f"{ordinal(t.layer)} layer"
                            break
                    try:
                        dt_obj = pd.to_datetime(from_date, dayfirst=True)
                        dt_minus_6 = dt_obj - timedelta(days=180)
                        date_minus_6_str = dt_minus_6.strftime('%d-%m-%Y')
                    except Exception as e:
                        logger.warning(f"Could not parse date {from_date}: {e}")
                        date_minus_6_str = from_date

                # Load Template
                if letter_type == 'victim':
                     template_name = 'Template for letter generation_victim account.docx'
                else:
                ...
                replacements = {
                    ...
                    '<Suspect Account Number>': str(account_number),
                    '<Suspect account number>': str(account_number),
                    '<suspect account number>': str(account_number),
                    '<Suspect Account>': str(account_number),
                }

                safe_replacements = {k: str(v).encode('ascii', 'replace').decode() for k, v in replacements.items()}
                ...
                if target_table:
                    ...
                    logger.warning("Transaction table not found in template, creating new one.")
                    target_idx = -1
                    for i, p in enumerate(doc.paragraphs):
                        if "Suspect Account Txn Details" in p.text:
                            target_idx = i
                            break
                    if target_idx != -1:
                ...
                if target_table:
                    for idx, t in enumerate(transactions, 1):
                        ...
                        if is_poh:
                            ...
                            row[1].text = str(t.to_account or '')
                ...
                base_dir = os.path.join(app.root_path, 'generated_letters')
                ack_dir = os.path.join(base_dir, folder_name)
                
                if is_poh or letter_type == 'suspect':
                    subfolder = 'suspect letter'
                elif letter_type == 'victim':
                    subfolder = 'victim letter'
                else:
                    subfolder = 'other letters'
                    
                target_dir = os.path.join(ack_dir, subfolder)
                os.makedirs(target_dir, exist_ok=True)
                
                prefix = "Suspect_Account_Letter" if is_poh else letter_type + "_Letter"
                filename = f"{prefix}_{account_number}.docx"
                safe_filename_val = secure_filename(filename)
                file_path = os.path.join(target_dir, safe_filename_val)
                
                doc.save(file_path)

                arcname = os.path.join(subfolder, safe_filename_val)
    ...
    try:
        ...
        for i, node in enumerate(nodes):
            ...
            if i == 0:
                ...
                # Suspect/Hold: Reddish
                fill_color = colors.HexColor("#F86262")
                header_text = f"Layer: {node.get('layer', 'N/A')}"
            else:
                # Intermediate: White/Grey
